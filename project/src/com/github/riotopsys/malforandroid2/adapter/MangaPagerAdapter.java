...
public class MangaPagerAdapter extends FragmentPagerAdapter {
	...
	@Inject
	public MangaPagerAdapter(FragmentManager fm, Context ctx) {
		super(fm);
		this.ctx = ctx;
	}
	...
	public Fragment getItem(int position) {
		...
		if ( MangaReadStatus.values().length > position ){
			args.putSerializable("filter", MangaReadStatus.values()[position]);
			f = new MangaListFragment();
		} else if ( MangaReadStatus.values().length == position )  {
	...
	@Override
	public int getCount() {
		return MangaReadStatus.values().length+2;//account for all and search
	}
	
	@Override
	public CharSequence getPageTitle(int position) {
		if ( MangaReadStatus.values().length > position ){
			return ctx.getString(MangaReadStatus.values()[position].getResource());
		} else if ( MangaReadStatus.values().length == position ) {
			//handle "all" condition
